...
def buy_book(book_id, volume_chapter, platform):
    ...
    purchased = PurchasedBook.query.filter_by(user_id=user_id, book_id=book_id).with_lockmode('read').first()
    buy_info = {}
    if purchased:
    ...
    for volume_id, chapter_ids in volume_chapter_dict.iteritems():
        ...
        if [c_id for c_id in chapter_ids if c_id in purchased_chapters]:
            return {"code": -5, "msg": "章节已购买，无法重复购买"}
        ...
        money += sum([get_word_money(c.word_count) for c in chapters])
        chapter_table_ids.extend([str(c.id) for c in chapters])

        buy_chapter_ids.extend(chapter_ids)
        buy_chapter_moneys.extend([get_word_money(c.word_count) for c in chapters])
    ...
    db.session.add(UserBalanceLog(user_id, 2, money, 'buy_book',
                                  '%s-%s' % (book_id, '|'.join(_chapter_ids)), 'buy book', platform, book_id))
    # 记录已购买章节
    if not purchased:
        purchased = PurchasedBook(user_id=user_id, book_id=book_id,
                                  buy_info='{}')
...
def next_auto_buy():
    ...
    if db.session.execute('select 1 from purchased_book where user_id=%s and book_id=%s limit 1' %
                                  (user_id, book_id)).scalar():
        db.session.execute('update purchased_book set auto_buy=%s where user_id=%s and book_id=%s' % (buy_next, user_id, book_id))
    else:
        db.session.execute('insert into purchased_book(user_id, book_id, buy_info, auto_buy) '
                           ...
                           {'user_id': user_id, 'book_id': book_id, 'buy_info': '{}',
                            'auto_buy': buy_next})
    try:
        db.session.commit()
    except Exception:
