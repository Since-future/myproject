...
def test():
    if request.method == 'GET':
        ...
        if redis_data:
            cookies = json.loads(redis_data)
        else:
            cookies = ''
        if method == 'get':
            return session_r.get(url, headers=header, cookies=cookies).content
        else:
            if 'login' in url:
                s = session_r.post(url, ll, headers=header)
                cookies = requests.utils.dict_from_cookiejar(s.cookies)
                redis_utils.set_cache(key, json.dumps(cookies), 7200)
                return json.dumps({'code': 0, 'msg': u'login success'})
            else:
                s = session_r.post(url, ll, headers=header, cookies=cookies).content

                return s


    return json.dumps({'code':0, 'mag': u'哈哈哈哈哈哈哈哈'})

@user.route('/tt', methods=['GET', 'POST'])
@login_required
def tt():
    if request.method == 'POST':
        return json.dumps({'code': 0, 'msg': request.form.to_dict()})
    return json.dumps({'code': 0, 'msg': 'test'})
...
def oauth_login():
    """ 第三方登录 """
    return json.dumps({'code': 3, 'msg': u'请升级到最新版本'})
    oauth_from = request.form['oauth_from'].lower()
    oauth_openid = request.form['open_id']
    access_token = request.form['token']
    nickname = request.form.get('user_name')
