...
def send(action):
    ...
    elif action in ('bind_phone', 'switch_user', 'modify_phone_verify', 'modify_phone', 'oauth_bind_cancel'):
        _action = action
        action = utils.get_define('SMSCAPTCHA_ACTION', action)
        if need_pic_captcha(_action):
            if not request.args.get('pic_captcha'):
                return json.dumps({'code': -999, 'msg': u'需要图形验证'})
            if not pic_captcha.validate_code(_action, request.args.get('pic_captcha', '')):
                return json.dumps({'code': -2, 'msg': u'图片验证码错误'})

        if _action in ('switch_user', 'modify_phone_verify'):
            if not user:
                return json.dumps({'code': 2, 'msg': u'用户不存在'})
        elif _action in ('bind_phone', 'modify_phone'):
            if user:
                return json.dumps({'code': 2, 'msg': u'该手机号已绑定帐号'})
        update_act_times(get_act_times_key(_action))
