...
from flask.ext.login import login_required, current_app
...
def add_book_shelf_name():
    ...
    bsn = BookShelfName(name, nickname)
    db.session.add(bsn)
    db.session.commit()
...
def update_book_shelf_name():
    ...
    if bsn:
        bsn.nickname = nickname
        bsn.update = datetime.now() 
        db.session.add(bsn)
        db.session.commit()
        return json.dumps({'code': 0, 'data': bsn.to_admin_dict()})
    else:
...
def book_shelf_books():
    name = request.form.get('name', '') or request.args.get('name', '')
    books = BookShelf.query.filter_by(name=name).all()
